...
bag_data = rosbag.Bag('generic.bag')
bridge = CvBridge()
K = np.matrix([[ 674.84,   0.00000000e+00,   298.61],[  0.00000000e+00,  674.01,  189.32],[  0.00000000e+00,   0.00000000e+00,   1.00000000e+00]])
d = np.array([0.159121, -0.549986, 0.003377, 0.001831, 0.000000])
...
for topic, msg, t in bag_data.read_messages(topics=['/ardrone/bottom/image_raw', '/vicon/ARDroneCarre/ARDroneCarre']):
    ...
    if topic == '/ardrone/bottom/image_raw':
        try:
            ...
            circles = cv.HoughCircles(img,cv.HOUGH_GRADIENT,1,20,param1=60,param2=40,minRadius=30,maxRadius=40)
...
print("C1: "+ str(np.average(C1, axis=0)))
print("C2: "+ str(np.average(C2, axis=0)))
print("C3: "+ str(np.average(C3, axis=0)))
print("C4: "+ str(np.average(C4, axis=0)))
print("C5: "+ str(np.average(C5, axis=0)))
print("C6: "+ str(np.average(C6, axis=0)))